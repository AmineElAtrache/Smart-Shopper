...
IKEA_BASE_URL = "https://www.ikea.com"
IKEA_SEARCH_URL = "https://www.ikea.com/ma/fr/search/?q={query}"
BROWSER_USER_AGENT = (
...
def build_search_url(task: ScrapeTaskAssigned) -> str:
    return IKEA_SEARCH_URL.format(query=quote_plus(_build_ikea_search_text(task)))
    ...
    for match in SCRIPT_JSON_RE.finditer(html):
        ...
        for item in _walk_json(payload):
            ...
            if title and price is not None and url:
                products.append(_raw_product(task, title=title, price=price, url=url, metadata={"parser": "json_ld", "category": "home"}))
    return products
        ...
        if title and price is not None:
            products.append(
                _raw_product(
                    ...
                    metadata={"parser": "dom_card", "category": "home"},
                )
            )
    return products
...
def _extract_title(anchor, block_text: str) -> str:
    ...
    if title and _parse_price(title) is None:
        return _clean_ikea_title(title)
    return _clean_ikea_title(clean_text(re.split(r"Prix|MAD|DH", block_text, maxsplit=1, flags=re.IGNORECASE)[0]))


def _clean_ikea_title(title: str) -> str:
    title = re.sub(r"^(Comparer|Option\s*:)\s*", "", clean_text(title), flags=re.IGNORECASE)
    title = re.sub(r"(?i)^comparer", "", title).strip()
    title = re.sub(r"(?i)^option\s*:\s*", "", title).strip()
...
def _clean_product_url(url: str | None) -> str | None:
    if not url or "ikea.com" not in url:
        return None
...
def _build_ikea_search_text(task: ScrapeTaskAssigned) -> str:
    query = task.query
...
def _matches_query(product: RawProduct, task: ScrapeTaskAssigned) -> bool:
    ...
    if query.product:
        product_terms = {query.product.lower(), PRODUCT_TERMS.get(query.product.lower(), query.product).lower()}
        if not any(term in searchable_text for term in product_terms):
            return False
