...
def build_search_url(task: ScrapeTaskAssigned) -> str:
    return ELECTROSALAM_SEARCH_URL.format(query=quote_plus(build_search_text(task)))
...
def _parse_dom_products(html: str, task: ScrapeTaskAssigned, *, page_url: str) -> list[RawProduct]:
    ...
    for anchor in soup.select('a[href*="/products/"]'):
        ...
        url = _clean_product_url(absolute_url(page_url, href))
        if not url:
            continue
...
def _nearest_product_block(anchor):
    ...
    for _ in range(6):
        ...
        if "/products/" in str(current) and _extract_sale_price(text) is not None:
            return current
...
def _extract_sale_price(text: str) -> float | None:
    sale_match = re.search(r"Prix de vente\s+(?P<price>[^\n]+?)(?:\s+Prix normal|$)", text, re.IGNORECASE)
    if sale_match:
...
def _extract_title(anchor, block_text: str) -> str:
    ...
    if title and _parse_price(title) is None:
        return title
    if "Prix de vente" in block_text:
        title = clean_text(block_text.split("Prix de vente", 1)[0])
        title = re.sub(r"^(Economisez\s+\d+%\s+)?(Livraison rapide\s+)?", "", title, flags=re.IGNORECASE)
...
def _clean_product_url(url: str | None) -> str | None:
    if not url or "electrosalam.ma" not in url:
        return None
