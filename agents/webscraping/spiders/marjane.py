...
MARJANE_BASE_URL = "https://www.marjanemall.ma"
MARJANE_SEARCH_URL = "https://www.marjanemall.ma/catalogsearch/result?q={query}"
BROWSER_USER_AGENT = (
...
def build_search_url(task: ScrapeTaskAssigned) -> str:
    return MARJANE_SEARCH_URL.format(query=quote_plus(build_search_text(task)))
...
async def _fetch_html(url: str, *, timeout: float) -> tuple[str, str]:
    if use_playwright_provider("marjane"):
...
def _extract_sale_price(text: str) -> float | None:
    marjane_price = _parse_marjane_display_price(text)
    if marjane_price is not None:
        return marjane_price
    prices = [_parse_price(match.group(0)) for match in PRICE_RE.finditer(text)]
    prices = [price for price in prices if price is not None]
    return prices[0] if prices else None


def _parse_marjane_display_price(text: str) -> float | None:
    price_matches = re.findall(r"(?<!\d)(?P<dirhams>\d{1,4})\s+(?P<cents>\d{2})\s*DH\b", clean_text(text), re.IGNORECASE)
    if not price_matches:
        return None
    dirhams, cents = price_matches[0]
    return float(f"{dirhams}.{cents}")
...
def _parse_price(value: str | None) -> float | None:
    ...
    if " " in raw_amount:
        ...
        if len(parts) == 2 and len(parts[-1]) == 2:
            raw_amount = f"{parts[0]}.{parts[1]}"
        else:
            raw_amount = "".join(parts)
    else:
...
def _extract_title(anchor, block_text: str) -> str:
    ...
    if title and _parse_price(title) is None and " DH" not in title:
        return title
    return _title_before_price_or_marker(block_text)


def _title_before_price_or_marker(text: str) -> str:
    title = clean_text(re.split(r"Vendu par|Prix|MAD|DH", text, maxsplit=1, flags=re.IGNORECASE)[0])
    title = re.sub(r"^Livraison rapide\s+", "", title, flags=re.IGNORECASE).strip()
    return title
...
def _clean_product_url(url: str | None) -> str | None:
    if not url or "marjanemall.ma" not in url:
        return None
    cleaned = url.split("?", 1)[0]
    if any(path in cleaned for path in ("/cart", "/checkout", "/login", "/search")):
        return None
    return cleaned
