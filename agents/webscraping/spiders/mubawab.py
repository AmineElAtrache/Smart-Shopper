"""Mubawab.ma real-estate scraper provider."""
...
MUBAWAB_BASE_URL = "https://www.mubawab.ma"
MUBAWAB_SEARCH_URL = "https://www.mubawab.ma/fr/sc/{query}"
BROWSER_USER_AGENT = (
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
    "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36"
)
PRODUCT_TERMS = {
    "apartment": "appartements-a-vendre",
    "apartments": "appartements-a-vendre",
    "appartement": "appartements-a-vendre",
    "villa": "villas-a-vendre",
    "house": "maisons-a-vendre",
    "maison": "maisons-a-vendre",
    "rent": "appartements-a-louer",
}
PRODUCT_MATCH_TERMS = {
    "apartment": {"appartement", "appartements", "apartment"},
    "apartments": {"appartement", "appartements", "apartment"},
    "appartement": {"appartement", "appartements", "apartment"},
    "villa": {"villa", "villas"},
    "house": {"maison", "maisons", "house"},
    "maison": {"maison", "maisons", "house"},
    "rent": {"louer", "location", "rent", "appartement"},
}
...
def build_search_url(task: ScrapeTaskAssigned) -> str:
    query = _build_mubawab_path(task)
    if task.query.city:
        return f"https://www.mubawab.ma/fr/st/{quote_plus(query, safe='/-')}"
    return MUBAWAB_SEARCH_URL.format(query=quote_plus(query, safe="/-"))
...
async def _fetch_html(url: str, *, timeout: float) -> tuple[str, str]:
    if use_playwright_provider("mubawab"):
    ...
    for match in SCRIPT_JSON_RE.finditer(html):
        ...
        for item in _walk_json(payload):
            ...
            if title and price is not None and url:
                products.append(_raw_product(task, title=title, price=price, url=url, metadata={"parser": "json_ld", "category": "real_estate"}))
    return products
...
def _parse_dom_products(html: str, task: ScrapeTaskAssigned, *, page_url: str) -> list[RawProduct]:
    ...
    for anchor in soup.select("a[href]"):
        ...
        if title and price is not None:
            products.append(_raw_product(task, title=title, price=price, url=url, metadata={"parser": "dom_card", "category": "real_estate"}))
    return products
...
def _raw_product(task: ScrapeTaskAssigned, *, title: str, price: float, url: str, metadata: dict[str, str]) -> RawProduct:
    return RawProduct(
...
def _clean_product_url(url: str | None) -> str | None:
    if not url or "mubawab.ma" not in url:
        return None
    cleaned = url.split("?", 1)[0]
    if not ("/fr/a/" in cleaned or "/fr/pa/" in cleaned):
        return None
    if any(path in cleaned for path in ("/fr/sc/", "/fr/search", "/fr/st/")):
        return None
    return cleaned


def _build_mubawab_path(task: ScrapeTaskAssigned) -> str:
    query = task.query
    product = PRODUCT_TERMS.get((query.product or "apartment").lower(), query.product or "appartements-a-vendre")
    city = (query.city or "").lower().replace(" ", "-").strip("-")
    return f"{city}/{product}" if city else product
