...
def materialize_llm_response(event: DecisionRanked, llm_text: str, *, fallback_message: str) -> str:
    ...
    if not event.products:
        ...
        if sections:
            general_reply = sections.get("general_reply") or sections.get("intro")
            if general_reply:
                cleaned = sanitize_llm_prose(general_reply, max_length=400)
                if _language(event) == "darija":
                    if is_coherent_darija(cleaned) and not _looks_like_hallucinated_listing(cleaned):
                        return cleaned
                elif is_usable_prose(cleaned, max_length=400) and not _looks_like_hallucinated_listing(cleaned):
                    return cleaned
...
HALLUCINATED_LISTING_PATTERN = re.compile(
    r"\b("
    r"jaweb[_\s-]?\d|khityarat|galaxy a\d+|iphone \d+|mafiawaystore|"
    r"kayna:\s*jumia|ma3lomat:\s*kayna"
    r")\b",
    re.IGNORECASE,
)


def _looks_like_hallucinated_listing(text: str) -> bool:
    return bool(HALLUCINATED_LISTING_PATTERN.search(text))
